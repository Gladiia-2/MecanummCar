...
extern uint16_t g_last_cnt[4];               // 上一次的编码器计数

extern volatile int16_t g_delta_cnt[4];      // 10ms 的计数增量
extern volatile float g_speed_rpm[4];        // 四个轮子的实际转速（rpm）
...
//		last_tick = now_tick;
    ...
    while (g_angle[0] < 0.0f)
    {
		...
		g_setspeed[0]= (location[0].out- location[1].out - k*vw)/(6.8f *PIE)*60.0f;   // 轮 1 目标转速 rpm		
    ...
    while (g_angle[1] < 0.0f)
    {
		...
		g_setspeed[1]= (location[0].out+location[1].out -k*vw)/(6.8f *PIE)*60.0f;   	
    ...
    while (g_angle[2] < 0.0f)
    {
		...
		g_setspeed[2]= (location[0].out-location[1].out+k*vw)/(6.8f *PIE)*60.0f;   
    ...
    while (g_angle[3] < 0.0f)
    {
		...
		g_setspeed[3]= (location[0].out+location[1].out+k*vw)/(6.8f *PIE)*60.0f;   
